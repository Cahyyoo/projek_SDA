...
function renderQueue() {
    ...
    queue.forEach((element) => {
        ...
        queueElement.textContent = `Elemen ${element.value}`;
...
enqueueButton.addEventListener('click', () => {
    ...
    statusMessage.textContent = `Elemen ${newElement.value} berhasil ditambahkan!`;
    currentNumber++;
});
...
dequeueButton.addEventListener('click', () => {
    ...
    const removedElement = queue[randomIndex];
    const removedElementNode = queueContainer.children[randomIndex];

    if (removedElementNode) {
        
        removedElementNode.classList.add('removing');
        setTimeout(() => {
    ...
    statusMessage.textContent = `Elemen ${removedElement.value} berhasil dihapus secara acak!`;
});
